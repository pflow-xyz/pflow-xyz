...
export class MetaModel {
    ...
    constructor(opts: ModelOptions) {
        ...
        if (opts.editor) {
            ...
            this.urlLoaded = loadModelFromUrl().then((urlModel) => {
                this.m = urlModel;
    ...
    async loadJson(json: string | object): Promise<boolean> {
        try {
            ...
            let data = json as ModelDeclaration;
            if (data) {
                if (!data.modelType) {
                    data.modelType = ModelType.petriNet;
                }
                if (!data.version) {
                    data.version = 'v0';
                }
            ...
            return compressBrotliEncode(this.toJson()).then((zipped) => {
