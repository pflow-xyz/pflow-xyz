...
type ModelResponse = {
    ...
    error: string;
    cid: string;
    declaration: mm.ModelDeclaration;
}

export async function loadModelFromUrl(): Promise<mm.Model> {
    ...
    if (!zippedJson) {
        ...
        let urlCid: string = getQueryParams()['cid'];
        if (urlCid.length === 49) {
            const m: ModelResponse = await fetch('/model/?cid=' + urlCid)
                .then((response) => {
