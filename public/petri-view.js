...
// NEW: drain the queue in strict order, exactly once at a time
    ...
    _draw() {
        ...
        arcs.forEach((arc, idx) => {
            ...
            if (curveOffset !== 0) {
                // For curved arcs, calculate the tangent at the end point
                const midX = (ex + fx) / 2;
                const midY = (ey + fy) / 2;
                const perpX = -uy;
                const perpY = ux;
            ...
            let bx, by;
            if (curveOffset !== 0) {
                // For curved arcs, position badge on the curve at t=0.5
                const midX = (ex + fx) / 2;
                const midY = (ey + fy) / 2;
                const perpX = -uy;
                const perpY = ux;
                const controlX = midX + perpX * curveOffset;
                const controlY = midY + perpY * curveOffset;
                // Quadratic bezier point at t=0.5: B(t) = (1-t)^2*P0 + 2(1-t)t*P1 + t^2*P2
                const t = 0.5;
